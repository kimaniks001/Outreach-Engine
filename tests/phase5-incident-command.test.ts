...
describe("True North Phase 5 incident command", () => {
  ...
  it("keeps incident visibility private until a responder is deliberately added", async () => {
    ...
    const responderEvents = (await listIncidentChronology(commander, id)).filter((entry) => entry.eventType === "RESPONDER_ADDED");
    expect(responderEvents).toHaveLength(1);
  });
  ...
  it("treats affected trader count as an operator estimate and tracks changes in chronology", async () => {
    ...
    await expect(updateIncidentImpact(commander, id, Number.NaN)).rejects.toThrow("non-negative integer");
    await expect(updateIncidentImpact(commander, id, 1.5)).rejects.toThrow("non-negative integer");
  });
  ...
  it("never turns communication state into publication authority", async () => {
    ...
    await setIncidentCommunicationState({ actorUserId: owner, incidentId: id, state: "RELEASED", releaseEvidenceRef: `approved:${run}` });
    await expect(setIncidentCommunicationState({ actorUserId: owner, incidentId: id, state: "DRAFTED" })).rejects.toThrow("immutable");
    await expect(setIncidentCommunicationState({ actorUserId: owner, incidentId: id, state: "RELEASED", releaseEvidenceRef: `replacement:${run}` })).rejects.toThrow("immutable");
  });
  ...
  it("links only support cases already visible to the incident responder", async () => {
    ...
    const linkEvents = (await listIncidentChronology(commander, id)).filter((entry) => entry.eventType === "SUPPORT_CASE_LINKED");
  ...
  it("turns prevention into normal Operations Work and keeps service signals evidence-only", async () => {
    ...
    await expect(recordServiceSignal({ actorUserId: commander, signalKey: `invalid-${run}`, serviceKey: "settlement", signalKind: "repeat-failure", evidenceRef: `${evidence}:invalid`, observedCount: Number.NaN })).rejects.toThrow("positive integer");
  });
});
