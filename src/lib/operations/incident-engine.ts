...
export async function openIncident(input: { actorUserId: string; title: string; summary?: string; severity: IncidentSeverity; affectedService: string; commanderUserId?: string; affectedTraderCount?: number; }): Promise<string> {
  ...
  const affected = Math.max(0, Math.trunc(input.affectedTraderCount ?? 0));
...
export async function addIncidentResponder(actorUserId: string, incidentId: string, responderUserId: string): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql`INSERT INTO operations_incident_responders (incident_id,user_id,added_by_user_id) VALUES (${incidentId}::uuid,${responderUserId}::uuid,${actorUserId}::uuid) ON CONFLICT DO NOTHING`);
...
export async function updateIncidentImpact(actorUserId: string, incidentId: string, affectedTraderCount: number): Promise<void> {
  await requireIncidentCommander(actorUserId, incidentId); const count = Math.max(0, Math.trunc(affectedTraderCount));
...
export async function linkSupportCase(actorUserId: string, incidentId: string, caseId: string): Promise<void> {
  ...
  await db.transaction(async (tx) => { await lockIncidentTx(tx,incidentId); await requireSupportCaseVisibilityTx(tx,actorUserId,caseId); await tx.execute(sql`INSERT INTO operations_incident_case_links (incident_id,case_id,linked_by_user_id) VALUES (${incidentId}::uuid,${caseId}::uuid,${actorUserId}::uuid) ON CONFLICT DO NOTHING`); await chronologyTx(tx,incidentId,"SUPPORT_CASE_LINKED",actorUserId,"A related trader support case was linked.",{ caseId }); });
}
...
export async function recordServiceSignal(input: { actorUserId: string; signalKey: string; serviceKey: string; signalKind: string; severityHint?: IncidentSeverity | null; evidenceRef: string; observedCount?: number; }): Promise<string> {
  await requireActiveStaff(input.actorUserId); const count=Math.max(1,Math.trunc(input.observedCount??1));
